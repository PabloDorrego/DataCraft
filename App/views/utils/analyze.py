...
class ChatGPT_Handler:  # designed for chat completion API
    ...
    def extract_output(self, text_input):
        ...
        for pattern in self.extract_patterns:
            ...
            elif "python" in pattern[1]:
                python_result = re.findall(pattern[1], text_input, re.DOTALL)
                if python_result:
                    output[pattern[0]] = python_result[0]
            else:
                other_result = re.search(pattern[1], text_input, re.DOTALL)
                if other_result:
                    output[other_result.group(1)] = other_result.group(2)
